...
\texttt{@rqb} & 8-bit quire (128-bit accumulator for \texttt{@rpb}) \\
\texttt{@rqh} & 16-bit quire (256-bit accumulator for \texttt{@rph}) \\
\texttt{@rqs} & 32-bit quire (512-bit accumulator for \texttt{@rps}) \\
\texttt{@rqd} & 64-bit quire (1024-bit accumulator for \texttt{@rpd}) \\
\texttt{@rqq} & 128-bit quire (2048-bit accumulator for \texttt{@rpq}) \\
\midrule
...
This deterministic numerical stack contributes several properties to the broader discussion of numerical determinism and reproducibility.  Determinism is a structural property:  while reproducible floating point is a known goal \citep{demmel2013reproducible,intel_cnr}, our contribution is the first-class enforcement mechanism.  (The co-design methodology itself is treated separately; see \citet{TODO} for a companion article.)
...
The library produces faithfully rounded functions rather than correctly rounded functions.\footnote{The distinction is that a faithfully rounded result is within 1~\textsc{ulp} of the exact result, while a correctly rounded result is the exact representable value closest to the exact result.  Faithful rounding is often sufficient for numerical applications and is generally faster to compute than correct rounding.  Truly correct rounding is often prohibitively expensive, especially for transcendental functions, and can require much higher intermediate precision to attain. TODO}
