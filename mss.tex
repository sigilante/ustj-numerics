...
\begin{table}[h]
\centering
\caption{SoftBLAS coverage by level (real-valued).  TODO}
\label{tab:coverage}
\begin{tabular}{@{}lll@{}}
\toprule
Level & Cost & Representative routines \\
\midrule
1 & $O(n)$   & \texttt{?dot}, \texttt{?axpy}, \texttt{?scal}, \texttt{?nrm2}, \texttt{?asum} \\
2 & $O(n^2)$ & \texttt{?gemv}, \texttt{?ger}, \texttt{?trmv}, \texttt{?trsv} \\
3 & $O(n^3)$ & \texttt{?gemm}, \texttt{?syrk}, \texttt{?trsm} \\
\bottomrule
\end{tabular}
\end{table}
...
Elementary functions are checked against \textsc{mpfr}~\citet{fousse2007mpfr} at high precision.  At half precision the input space is $2^{16}$ values and the test is exhaustive; at single precision the $2^{32}$ space is likewise tested in full.  At double and quadruple precision, where exhaustive testing is infeasible, we report maximum observed unit of least precision TODO (\textsc{ulp}) error over a sampled and edge-weighted input set rather than a guarantee.  We claim a faithful-rounding ($\leq 1$~\textsc{ulp}) result where the testing supports it and a measured bound where it does not.
...
The \texttt{/lib/unum} transcendentals are currently implemented as Taylor series expansions like the legacy \texttt{/lib/math} operations.  Results are included in Table~\ref{tab:unum}.
TODO it's a moral duty to implement Chebyshev here for comparison

\begin{landscape}
\begin{table}[h]
\centering
\caption{Elapsed times and relative slowdowns of unum jetted Taylor series functions for 32-bit \texttt{@rps}/\texttt{posit32} atom operations with \texttt{/lib/unum} and interpreted Taylor series functions.  Slowdown is reported relative to the jetted functions.}
\label{tab:unum}
\begin{tabular}{lrrrrr}
\toprule
Arm & Taylor (jet) & Taylor (int.) & T.\ slowdown \\
\midrule
\texttt{+add} & 1.689 µs & 374.74 µs & 222× \\
\texttt{+sub} & 1.757 µs & 339.75 µs & 193× \\
\texttt{+mul} & 1.622 µs & 276.50 µs & 170× \\
\texttt{+div} & 1.443 µs & 344.33 µs & 239× \\
\texttt{+fma} & 1.897 µs & 492.67 µs & 260× \\
\texttt{+sqt} & 17.304 µs & 341.57 µs & 20× \\
\texttt{+neg} & 0.735 µs & 19.68 µs & 27× \\
\texttt{+lth} & 1.281 µs & 20.80 µs & 16× \\
\texttt{+exp} & 25.315 µs & 19426.71 µs & 767× \\
\texttt{+log} & 39.866 µs & 39066.79 µs & 980× \\
\texttt{+sin} & 34.752 µs & 32292.92 µs & 929× \\
\texttt{+cos} & 35.678 µs & 32480.51 µs & 910× \\
\texttt{+atan} & 850.269 µs & 63708.31 µs &  75× \\
\texttt{+pow} & 60.657 µs & 56962.95 µs & 939× \\
\texttt{+fdp} & 0.270 µs & 205.49 µs & 761× \\
\bottomrule
\end{tabular}
\end{table}
\end{landscape}
